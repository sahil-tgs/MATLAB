...
for ii=1:length_of_sqn
    fliped_y(ii)=h(length_of_sqn-ii+1);
end
...
indexing_new=...
    length(fliped_y_with_appneded_zeros);

x_new=[post_fix_x, x, post_fix_x];
con_output=zeros...
    (1, (length(fliped_y)+2*appned_zeros) );

subplot(4,1,1),plot(t(1:ii), x(1:ii));
subplot(4,1,2),plot(t(1:ii), h(1:ii));
